...
import io.github.alelk.apps.challengetgbot.di.testModule
...
class ChallengeRepositoryTest : FunSpec(), KoinTest {
    ...
    init {
        ...
        afterEach {
            get<DatabaseService>().dropAllTables()
            stopKoin()
        }
        ...
        test("savePollAnswer should save new answer") {
            val repository = get<ChallengeRepository>()
            val challengeId = repository.saveChallenge(
                ChallengeEntity(
                    groupName = "TestGroup",
                    pollId = "poll789",
                    messageId = 12345L,
        ...
        test("getUserStatistics should handle multiple users") {
            val repository = get<ChallengeRepository>()
            val challenge1Id = repository.saveChallenge(
                ChallengeEntity(
            ...
            val challenge2Id = repository.saveChallenge(
                ChallengeEntity(
                    groupName = "MultiGroup",
                    pollId = "poll_multi2",
                    messageId = 2L,
